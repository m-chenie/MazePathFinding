...
def create_grid():  # creates 2D array that represents grid
...
def draw_cells(grid):  # draws grid
    for row in range(ROWS):
        for col in range(COLS):
...
def astar(grid, start, end):
    ...
    for row in grid:
        for cell in row:
            f[cell] = float("inf")
    f[start] = h(start.row, start.col, end.row, end.col)
    ...
    while not open_set.empty():
        ...
        for neighbor in curr.neighbors:
            ...
            temp_g_score = g[curr] + 1  # adding 1 to g_score to simulate movement of curr to any adj cell
            if temp_g_score < g[neighbor]:
                g[neighbor] = temp_g_score
                f[neighbor] = temp_g_score + h(neighbor.row, neighbor.col, end.row, end.col)
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if pygame.mouse.get_pressed()[0]:  # left click to set start cell
                ...
                if start_cell is None and selected_cell.path == True and selected_cell.end == False:
            ...
            if pygame.mouse.get_pressed()[2]:  # right click to set end cell
                ...
                if end_cell is None and selected_cell.path == True and selected_cell.start == False:
